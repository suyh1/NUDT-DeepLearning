...
def test():
    ...
    train_batches = train_ds.train_ds.shuffle(SHUFFLE_BUFFER_SIZE).batch(BATCH_SIZE).repeat().prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
    val_batches = val_ds.shuffle(SHUFFLE_BUFFER_SIZE).batch(BATCH_SIZE).repeat().prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
    test_batches = test_ds.shuffle(SHUFFLE_BUFFER_SIZE).batch(BATCH_SIZE).repeat().prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
    ...
    layer_name = 'block_16_expand_relu' # 选择一个中间层用于进行关键点位置回归预测。中间层的layer_name可以通过mobile_net.summary()查看，任务说明中也已经给出。由于在线平台调试信息显示行数的限制，不推荐在线使用mobile_net.summary()，可以在自己的设备上进行测试
    ...
    lm_pred = tf.keras.layers.Dense(14, name='lm_pred')(y)# 关键点位置预测层，每张图片均有7个关键点，每个关键点要预测(x,y)坐标
    ...
    mt_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=lr_schedule),
                     loss={
                         'c_pred': 'categorical_crossentropy',
                         'lm_pred': 'mean_absolute_error'}, # 为两个任务选择合适的loss
                     loss_weights={
                         'c_pred': 1.,
                         'lm_pred': 1.
    ...
    mt_model.fit(train_batches, epochs=8, validation_data=val_batches,
                 steps_per_epoch=steps_per_epoch, validation_steps=v_steps_per_epoch, callbacks=[tensorboard_callback], verbose = 2)
